...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if ($application_id && is_numeric($application_id) && !empty($name) && !empty($comment_text)) {
        ...
        if ($stmt->execute()) {
            header("Location: view_application.php?id=" . urlencode($application_id) . "&message=Comment+successfully+added!");
        } else {
            header("Location: view_application.php?id=" . urlencode($application_id) . "&error=Error+add+comment:+" . urlencode($stmt->error));
        }
        $stmt->close();
    } else {
        header("Location: index.php?error=Invalid+submitt+comment.");
    }
} else {
